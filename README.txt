README - Baptiste Lemonnier - Projet Ubiquity
...
que développeur, de savoir faire un site de vente de produits en utilisant les meilleurs outils permettant de créer
un site attractif.

Le framework Ubiquity est le framework php idéal pour pouvoir accomplir cette tâche.
Il fait gagner beaucoup de temps, car le code est mieux concentré entre les différentes pages permettant ainsi d'obtenir
...
Pour commencer à travailler sur le projet, il faut installer et utiliser la version 8.0.0 ou une version plus récente, il faut aussi évidemment
...
Une fois les installations faite ci-dessus, il faut :
...
Dans Ubiquity, nous avons :
 ...
 - Les models, permettant d'initaliser des variables, de générer les getters et setters
 - Les views, permettant d'afficher des éléments préparés par les controllers avec du design.
...
/ : page "dashboard" permettant d'avoir un récapitulatif de nos interventions sur le site
/order : historique des commandes
/store : visualisation du nombre de produit dans les sections / permet de parcourir les sections
...
/basket : permet de voir le contenu des paniers déjà créés
/product/x/y/ : permet de voir la fiche complète d'un produit
/basket/validate : permet de valider le panier
/basket/command : permet de valider la commande
/basket/clear : permet de vider le panier